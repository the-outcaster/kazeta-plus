...
/*
/// Scans potential mount points to find the path to a cart.kzi file.
fn find_cart_kzi() -> Result<PathBuf, SaveError> {
    // Create a list of common Linux media mount points to check
    let potential_bases = vec![
        format!("/run/media/{}", whoami::username()),
            "/media".to_string(),
    ];

    // Loop through each potential base path
    for base in potential_bases {
        ...
        // Scan the subdirectories of the current base path
        if let Ok(entries) = fs::read_dir(base_path) {
            for entry in entries.flatten() {
                let mount_path = entry.path();
                if mount_path.is_dir() {
                    let kzi_path = mount_path.join("cart.kzi");
                    if kzi_path.exists() && kzi_path.is_file() {
                        // Found it! Return the path immediately.
                        println!("[Debug] Found cart.kzi at: {}", kzi_path.display());
                        return Ok(kzi_path);
                    }
                }
            }
    ...
    // If the loop finishes, no cart was found in any of the locations.
    Err(SaveError::Message("No 'cart.kzi' file found in any known mount point.".to_string()))
}
*/

/*
/// Detects a game cartridge and parses its .kzi metadata file.
pub fn get_cart_info() -> Result<(CartInfo, PathBuf), SaveError> {
    // 1. Find the path to the cart.kzi file. `?` will propagate the error if not found.
    let kzi_path = find_cart_kzi()?;

    // 2. Read the file contents. `?` will propagate an IO error.
    let content = fs::read_to_string(&kzi_path)?;
    ...
    // 4. Validate and construct the CartInfo struct
    if let (Some(id), Some(exec), Some(icon)) = (id, exec, icon) {
        Ok((
            CartInfo { name, id, exec, icon, runtime },
            kzi_path,
        ))
    } else {
        // Return a specific error if required fields are missing
        Err(SaveError::Message(format!("Invalid .kzi file: '{}'. Missing one or more required fields (Id, Exec, Icon).", kzi_path.display())))
    }
}
*/
